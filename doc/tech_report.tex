...
\begin{equation}
\Delta T_{ij} \equiv \frac{\partial S^b_{\rm{full}}}{\partial T_{ij}} = \sum_{t=1}^{S-1} \Delta I_{tbi}\frac{e^{W^{t-1}_{bji}+\alpha^{t-1}_{bj}}}{\sum_k e^{W^{t-1}_{bki}+\alpha^{t-1}_{bk}}}.
\end{equation}
...
\end{equation}
note that this is $0$ if $\bar\gamma^t_{bs}=-\infty$, so inaccessible states do not contribute. 
\begin{align}
...
&& t>0, s>0\\
\frac{\partial \bar\alpha^t_{b0}}{\partial \bar{V}^{t-1}_{b,s-1}}
